...
export default async function RegisterPage({ searchParams }: { searchParams: Promise<{ error?: string; type?: string }> }) {
  ...
  const registrationType = params.type === "organization" ? "ORGANIZATION" : params.type === "family" ? "FAMILY" : "INDIVIDUAL";

  return (
    <main className="mx-auto grid max-w-xl gap-6 px-4 py-12">
      <div>
        <h1 className="text-3xl font-extrabold text-ink">Opret profil</h1>
        <p className="mt-2 text-slate-600">Vælg det medlemskab, der passer til dig.</p>
      </div>
      <form action={registerAction} className="grid gap-4 rounded-[32px] border-2 border-fjord/25 bg-white p-6 shadow-sm">
        ...
        <fieldset className="grid gap-3">
          <legend className="text-sm font-bold text-ink">Medlemskab</legend>
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="flex items-start gap-3 rounded-2xl border border-fjord/30 bg-cream p-4">
              <input className="mt-1 h-4 w-4" type="radio" name="registrationType" value="INDIVIDUAL" defaultChecked={registrationType === "INDIVIDUAL"} />
              <span>
                Borger
                <span className="block text-xs font-normal text-slate-600">Personligt medlemskab.</span>
              </span>
            </label>
            <label className="flex items-start gap-3 rounded-2xl border border-fjord/30 bg-cream p-4">
              <input className="mt-1 h-4 w-4" type="radio" name="registrationType" value="FAMILY" defaultChecked={registrationType === "FAMILY"} />
              <span>
                Familie
                <span className="block text-xs font-normal text-slate-600">Medlemskab for familien.</span>
              </span>
            </label>
            <label className="flex items-start gap-3 rounded-2xl border border-fjord/30 bg-cream p-4">
              <input
                ...
                value="ORGANIZATION"
                defaultChecked={registrationType === "ORGANIZATION"}
              />
              <span>
                Forening
