...
export async function updateAdminRideAction(rideRequestId: string, formData: FormData) {
  ...
  if (!parsed.success) redirect(`/dashboard/admin/rides/${rideRequestId}?error=${encodeURIComponent(parsed.error.issues[0].message)}`);

  const result = await updateRideDetails(rideRequestId, parsed.data, bus);
  if ("error" in result) redirect(`/dashboard/admin/rides/${rideRequestId}?error=${encodeURIComponent(result.error ?? "Turen kunne ikke opdateres.")}`);

  if (result.ride.isSharedRide && result.shift) await ensureRideSharingEvent(result.ride.id);
  else await updateRideSharingEventStatus(result.ride.id, "CANCELLED");
  ...
  if (updatedRide.assignment?.driverProfile.userId) recipientIds.add(updatedRide.assignment.driverProfile.userId);
  updatedRide.sharedEvent?.signups.forEach((signup) => recipientIds.add(signup.citizenProfile.userId));
  await createNotifications(Array.from(recipientIds).map((userId) => ({
    userId,
  ...
  await createAuditLog({ actorUserId: admin.id, action: "RIDE_UPDATED", entityType: "RIDE_REQUEST", entityId: updatedRide.id, description: `${admin.name} redigerede ${updatedRide.citizenProfile.user.name}s tur den ${updatedRide.rideDate.toLocaleDateString("da-DK")} kl. ${updatedRide.rideTime}.` });

  ["/dashboard/admin", "/dashboard/admin/shifts", "/dashboard/admin/buses", "/dashboard/admin/events", "/dashboard/admin/activity", "/dashboard/citizen", "/dashboard/citizen/events", "/dashboard/driver", "/"].forEach((path) => revalidatePath(path));
