            ...
            <article key={event.id} className="rounded-[28px] border-2 border-fjord/20 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-extrabold text-ink">{event.title}</h2>
                  ...
                  <div className="grid gap-4 sm:grid-cols-[1fr_160px]">
                    <div className="grid gap-2">
                      ...
                      <input
                        id={`pickup-${event.id}`}
                        name="pickupAddress"
                        defaultValue={user.citizenProfile?.address ?? ""}
                        placeholder="Hvis du vil hentes et bestemt sted"
                      />
                    </div>
                    <div className="grid gap-2">
                      <label htmlFor={`passengers-${event.id}`}>Antal</label>
                      <input id={`passengers-${event.id}`} name="passengers" type="number" min="1" max="8" defaultValue="1" />
                    </div>
                  </div>
                  <div className="grid gap-2">
                    <label htmlFor={`notes-${event.id}`}>Note</label>
