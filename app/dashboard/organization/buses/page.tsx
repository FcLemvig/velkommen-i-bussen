    ...
    <main className="mx-auto grid max-w-6xl gap-6 px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-slate-600">Se hvornår Bus Øst og Bus Vest er optaget.</p>
        </div>
        <Link href="/dashboard/organization" className="button gap-2 border-2 border-fjord/30 bg-white text-ink hover:bg-cream">
          <ArrowLeft size={16} />
          Tilbage
        </Link>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Optagede tider vises uden navn, chauffør eller formål.</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href={`/dashboard/organization/buses?week=${previousWeek}`} className="button gap-2 border-2 border-fjord/30 bg-white text-ink hover:bg-cream">
              <ChevronLeft size={16} />
              Forrige uge
            </Link>
            <Link href={`/dashboard/organization/buses?week=${thisWeek}`} className="button border-2 border-fjord/30 bg-white text-ink hover:bg-cream">
              Denne uge
            </Link>
            <Link href={`/dashboard/organization/buses?week=${nextWeek}`} className="button gap-2 bg-bus text-white hover:bg-bus/90">
              Næste uge
                ...
                return (
                  <div key={`${bus}-${day.toISOString()}`} className="min-h-36 border-l border-slate-100 p-3">
                    <div className="grid gap-2">
                      ...
                      {dayShifts.map((shift) => (
                        <div key={shift.id} className="rounded-2xl border border-fjord/30 bg-fjord/10 px-3 py-2 text-sm text-ink">
                          <div className="font-bold">
